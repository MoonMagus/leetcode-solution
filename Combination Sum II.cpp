...
public:
    vector<vector<int>> combinationSum2(vector<int>& candidates, int target) {
        ...
        dfs(results, candidates, path, target, 0);

        return results;
    }

private:
    void dfs(vector<vector<int>>& results, vector<int>& candidates, vector<int>& path, int gap, int start) {
        if (gap == 0) {
        ...
        for (int i = start; i < candidates.size(); ++i) {
            ...
            dfs(results, candidates, path, gap - candidates[i], i + 1);
            path.pop_back();
        }
    }
};
