...
public:
    int maxProfit(vector<int>& prices) {
        ...
        vector<int> L(n, 0);
        vector<int> R(n + 1, 0);
        // Caculate lowest prices from left to right.
        for (int i = 1, lowest = prices[0]; i < n; ++i) {
            L[i] = max(L[i - 1], prices[i] - lowest);
            lowest = min(lowest, prices[i]);
        }

        // Caculate lowest prices from right to left.
        for (int i = n - 2, highest = prices[n - 1]; i >= 0; --i) {
            R[i] = max(R[i + 1], highest - prices[i + 1]);
