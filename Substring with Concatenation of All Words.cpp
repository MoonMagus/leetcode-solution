...
public:
    vector<int> findSubstring(string s, vector<string>& words) {
        ...
        const int words_size = word_count * word_size;

        // Initialize word map.
        unordered_map<string, int> table;
        unordered_map<string, int> current_table;
        for (auto w : words)
            table[w]++;

        for (int i = 0; i + words_size - 1 < ls; ++i) {
            current_table = table;
            int count = 0;
            for (int j = 0; j < words_size; j += word_size) {
                string w = s.substr(i + j, word_size);
                if (current_table.find(w) != current_table.end() && current_table[w] > 0) {
                    current_table[w]--;
                    count++;
                }
                else
