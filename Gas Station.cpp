...
public:
    int canCompleteCircuit(vector<int>& gas, vector<int>& cost) {
        ...
        int num_cost = cost.size();
        // If input data is not illegal, then return -1.
        if (num_gas != num_cost || num_gas == 0)
            return -1;

        int total = 0;
        int index = -1;
        for (int i = 0, behind = 0; i < num_gas; ++i) {
            behind += gas[i] - cost[i];
            total += gas[i] - cost[i];
            if (behind < 0) {
                behind = 0;
