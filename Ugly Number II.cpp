...
public:
    int nthUglyNumber(int n) {
        ...
        int count = 1;
        while (count < n) {
            int next = minOfThree(uglyNums[bigger2] * 2, uglyNums[bigger3] * 3, uglyNums[bigger5] * 5);
            ++count;
            uglyNums[count - 1] = next;

            while (uglyNums[bigger2] * 2 <= uglyNums[count - 1])
                ++bigger2;
            while (uglyNums[bigger3] * 3 <= uglyNums[count - 1])
                ++bigger3;
            while (uglyNums[bigger5] * 5 <= uglyNums[count - 1])
                ++bigger5;
        }
