...
public:
    int kthSmallest(TreeNode* root, int k) {
        ...
        int checked = 0;
        int result;
        kthSmall(root, k, checked, result);

        //if (checked != k)
        //    throw exception("Tree nodes count less than k");

        return result;
    }

private:
    void kthSmall(TreeNode* root, int k, int& checked, int& result) {
        // Check left subtree.
        if (root->left)
            kthSmall(root->left, k, checked, result);
        ...
        // Check right tree.
        if (root->right)
            kthSmall(root->right, k, checked, result);
    }
};
